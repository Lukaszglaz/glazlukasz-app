    ...
    >
      <div className="xl:max-w-8xl mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="mt-16 grid grid-cols-1 gap-6 lg:grid-cols-3">
          {withdrawalSteps.map((box, i) => (
            <div
              key={i}
              className={`rounded-[2.5rem] border border-border p-10 transition-all ${box.highlight ? "border-accent/30 bg-accent/5" : "bg-bg-card"}`}
            >
              <div
                className={`mb-8 flex h-12 w-12 items-center justify-center rounded-2xl ${box.highlight ? "bg-accent text-bg-body" : "bg-accent/10 text-accent"}`}
              >
                <box.icon size={24} />
              </div>
              ...
              <h3
                className={`mb-4 text-sm font-extrabold tracking-tighter uppercase ${box.highlight ? "text-accent" : "text-text-primary"}`}
              >
                {box.val}
              </h3>
        ...
        <section className="mt-40 space-y-10">
          <div className="flex flex-col justify-between gap-6 px-2 md:flex-row md:items-end">
            <div>
              ...
              <p className="mt-4 text-[11px] font-bold tracking-[0.2em] text-tertiary uppercase">
                Wymagany do procesowania rezygnacji
              </p>
            </div>
            <button
          ...
          <div className="overflow-hidden rounded-[2.5rem] border border-border bg-bg-card shadow-2xl">
            <div className="divide-y divide-border/30">
              {withdrawalSectionsData.map((section) => (
                <div
                  key={section.id}
